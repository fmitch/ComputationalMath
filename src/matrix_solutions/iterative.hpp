...
static int c = 0;
double* jacobi(double* A, double* b, int N, int& count = c, long double tol=1e-8, int maxIter = 1e6, double* guess = nullptr);
double* gaussSeidel(double* A, double* b, int N, int& count=c, long double tol=1e-8, int maxIter = 1e6, double* guess = nullptr);
